    ...
    while open:
        ...
        if pop_g == state_cost[pop_state]:
            ...
            #     already reach the state in a cheaper way discovered in the past.
                ...
                for i in task.get_successor_states(pop_state):
                    neighbor = searchspace.make_child_node(pop_node, i[0], i[1])
                    neighbor_h = heuristic(neighbor)
                    if neighbor_h == float('inf'):
                        continue
                    elif neighbor.state not in state_cost or neighbor.g < state_cost[neighbor.state]:
                        ...
                        node_tiebreaker += 1
                        state_cost[neighbor.state] = neighbor.g
                        heapq.heappush(open, make_open_entry(neighbor, h, node_tiebreaker))
                    else:
                        continue
